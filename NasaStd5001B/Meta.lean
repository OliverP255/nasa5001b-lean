/-
  ...
  A. CONCRETE — facts about the numeric values in Table 1.
     Proved by `native_decide` or `rfl`; no hypotheses required.
...
theorem prototype_higher_qual :
    qualTestFactor .protoflight < qualTestFactor .prototype := by native_decide
...
theorem protoflight_higher_yield :
    designFactor .prototype .yield < designFactor .protoflight .yield := by native_decide
...
theorem all_design_factors_pos : ∀ a : TestApproach, ∀ m : FailureMode,
  ...
  intro a m; cases a <;> cases m <;> native_decide
...
theorem proofTestFactor_gt_one : 1 < proofTestFactor := by native_decide
...
theorem proflight_yield_implies_proto_yield {s : StructuralCheck}
  ...
  exact le_trans h (margin_antitone_factor hσ (by native_decide) hallow (by native_decide))
