...
/-- Qualification test factors (from Table 1)
...
/-- Proof test factor from Table 1 footnote ** (§4.2.1).
    ...
    cases.  For all other metallic structures no proof test is required. -/
...
/-- Margin of Safety as defined in §3.2:
      ...
      · allowable = material allowable stress
...
/-- All the inputs needed to check structural adequacy against NASA-STD-5001B §4.2d.

    Fields:
      · σ_max = upper bound on max von Mises stress at limit
                        load (MPa).
